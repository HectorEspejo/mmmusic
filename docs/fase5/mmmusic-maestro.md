...
**Última actualización:** 13 de septiembre de 2026 (Fase 5 especificada; Fase 3 completada y validada; Fase 4 pendiente de especificar)
...
└──────────────────────────────┬────────────────────────────────────────────┘
       ...
       ▼                       ▼                       ▼                  ▼                  ▼
┌──────────────┐   ┌──────────────────────┐   ┌──────────────────────┐   ┌──────────────────────┐   ┌─────────────────┐
│ FASE 2 ✔     │   │ FASE 3 ✔             │   │ FASE 4 (pendiente)   │   │ FASE 5               │   │ FASE 6          │
│ Scrobbling,  │   │ Visuales reactivas   │   │ Letras + ecualizador │   │ Radio y streams      │   │ Nicotine+       │
│ recopilat.,  │   │ (PipeWire, FFT,      │   │ (mpv af, PISTAS)     │   │ (cola mixta, ICY,    │   │ (escaneo notify)│
...
| 4 | Ecualizador y Letras | — | — | — | Propuesta, pendiente de especificar |
| 5 | Radio y Streams | 60 | 4 | 3-4 | Especificada |
| **Total** | | **290** | **17** | **~4,5 meses** | |
...
| 4 | — | sin especificar |
| 5 | 0 / 60 | ninguno — sin confirmar |

**Progreso total:** 230 / 290 funcionalidades.

Pendiente de decisión: ubicación de la documentación (`docs/` plano según `CLAUDE.md` frente a `docs/faseN/` que crea el agente).

## 7. Roadmap

| Fase | Contenido | Depende de |
|------|-----------|-----------|
| 4 | Letras (ficheros `.lrc` junto a la pista y etiqueta `lyrics`) y ecualizador (filtros `af` de mpv con presets); posible vista letras + visual. **Saltada por ahora**, se especifica cuando Hector lo pida | F1 (envoltorio mpv), F3 |
...
| — | Ideas sin fase: edición de etiquetas, podcasts (RSS), grabación de streams, emisoras en Inicio, espectro espejado, "sunset grid" vaporwave, carátula pulsante, exportar frame de visual, mini visual en Waybar, arranque perezoso de la captura (R-15) | |

Orden recomendado tras la 5: 6 → 4 → 7 → 8 → 9 (la 4 puede intercalarse cuando se quiera). Idea añadida en F2: sincronizar favoritas con ListenBrainz si algún día la biblioteca lleva MBIDs (Picard).
...
| R-16 | La migración 004 recrea tres tablas con datos reales (cola, historial, envíos) | Pérdida de historial si falla a medias | Transacción + copia `mmmusic.db.pre-004` + test con datos de F2 |
