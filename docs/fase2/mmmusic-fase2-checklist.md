...
- [ ] Carga de `~/.config/mmmusic/credenciales.toml` con secciones `[listenbrainz]` (token) y `[lastfm]` (api_key, api_secret, session_key, usuario)
- [ ] Permisos de `credenciales.toml` distintos de 600 se corrigen automáticamente con toast de aviso
- [ ] Servicio activo en config sin credenciales se trata como inactivo con toast al arrancar
...
- [ ] Códigos de salida de CLI: 0 OK, 1 error de configuración, 2 error de red
...
- [ ] Regla de elegibilidad: pista > 30 s; umbral min(50 %, 4 min) de tiempo real acumulado
- [ ] `HISTORIAL_REPRODUCCION.completada` pasa a usar el umbral min(50 %, 4 min) en lugar del 50 % fijo
- [ ] Now playing enviado a cada servicio activo al cargar una pista elegible, sin cola ni reintentos
- [ ] Encolado de un ENVIO `scrobble` por servicio activo al marcarse `completada`, con `reproducido_en` = inicio de la escucha
- [ ] Ciclo de envío cada 30 s con lotes de hasta 50 pendientes por servicio ordenados por `reproducido_en`, y reposo del hilo sin pendientes
- [ ] Planificador de reintentos exponencial 30 s · 2^intentos con tope 6 h y descarte tras 20 intentos
- [ ] Errores de autenticación (401/403, Last.fm 4/9/14) reprograman a +24 h, guardan `scrobbling_ultimo_error` y muestran toast con el servicio afectado
...
- [ ] Lote ListenBrainz rechazado (400) se reintenta elemento a elemento para aislar y descartar el defectuoso
- [ ] Toda petición HTTP con `ureq` (rustls), timeout 10 s, `User-Agent: mmmusic/<versión>`, solo a `api.listenbrainz.org` y `ws.audioscrobbler.com`
- [ ] Tokens, `api_secret` y `session_key` nunca aparecen en logs, toasts ni `error_msg`
- [ ] Los envíos pendientes sobreviven al cierre y se envían en el siguiente arranque
...
## ListenBrainz
- [ ] `POST /1/submit-listens` con `listen_type = playing_now` (sin `listened_at`) y `import` (con `listened_at` en segundos Unix), cabecera `Authorization: Token`
- [ ] `track_metadata` con artist_name, track_name, release_name y `additional_info` (duration_ms, media_player "mmmusic", submission_client "mmmusic")
- [ ] `GET /1/validate-token` para `probar-servicios`
...
## Interfaz
- [ ] Indicador de scrobbling en la barra inferior: `↑` enviado, `…N` pendientes, `!` error, nada si está desactivado, con equivalentes `sc:ok` / `sc:N` / `sc:err` en modo ascii
- [ ] En modo compacto (< 70 columnas) el indicador solo muestra `!` si hay error
- [ ] `Ctrl+s` fuerza el envío inmediato de pendientes
...
- [ ] Cabecera de álbum y vista Artistas muestran "Varios artistas" en los recopilatorios, manteniendo el artista real por pista en la lista
...
- [ ] Test de integración de recopilatorios con `tests/fixtures/recopilatorio/` (3 pistas, artistas distintos, sin albumartist) y de su reversión al etiquetar `albumartist`
- [ ] Test de persistencia de ENVIOS y de descarte de love/unlove anteriores
...
**Progreso Fase 2:** 14 / 59 funcionalidades

**Total mmmusic (Fases 1-2):** 126 / 171 funcionalidades
